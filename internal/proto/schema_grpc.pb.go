...
const (
	Chat_Join_FullMethodName = "/grpc.Chat/Join"
)
...
func (c *chatClient) Join(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Message, Message], error) {
...
type Chat_JoinClient = grpc.BidiStreamingClient[Message, Message]
...
func (UnimplementedChatServer) Join(grpc.BidiStreamingServer[Message, Message]) error {
...
func _Chat_Join_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ChatServer).Join(&grpc.GenericServerStream[Message, Message]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Chat_JoinServer = grpc.BidiStreamingServer[Message, Message]
